import { ATHLETES, ARENAS, AI_OPPONENTS, COURT, isUnlocked, outfitChallengeMet, seasonObjectives, matchObjective, OBJECTIVE_DEFS, UNLOCK_CODE, outfitsForAthlete, SEASON_METRIC_AGG, emptySeasonProgress, CAREER_MATCHES, CAREER_PROMOTION_WINS, CAREER_FINAL_SEASON, careerAiProfile, careerFixture, tournamentFixture, VERSION, FEEDBACK, FEEDBACK_TOPICS } from "./data.js?v=20260910-sprite-gate-v41";
import { getMatchInfo } from "./game.js?v=20260910-sprite-gate-v41";
import { getVolume, isMuted } from "./audio.js?v=20260910-sprite-gate-v41";
import { getLang, t } from "./i18n.js?v=20260910-sprite-gate-v41";
import { BUILD, IS_DEMO, DEMO_CONTENT, demoFilter, demoLocked } from "./build.js?v=20260910-sprite-gate-v41";
...
export function applyDemoLimits() {
  ...
  // Il badge deve dire cosa sta girando: chiamare "demo" una beta con tutte le
  // difficolta' sbloccate sarebbe falso, e chi prova non capirebbe cosa gli manca.
  document.querySelectorAll("[data-demo-badge]").forEach((el) => {
    el.textContent = t(BUILD === "beta" ? "betaBadge" : "demoBadge");
    el.hidden = false;
  });
  ...
  // `difficulty: null` significa "nessun blocco". La demo ne concede una sola — il
  // facile fa sembrare il gioco banale, il difficile respinge chi ha in mano il
  // controller da tre minuti — ma la beta le vuole tutte, perche' esiste per
  // raccogliere feedback proprio su Difficile e Leggenda.
  if (!DEMO_CONTENT.difficulty) return;
...
export function renderChallenges() {
  ...
  // Il testo deve dire quale build gira: chiamare "demo" una beta con tutte le
  // difficolta' sbloccate confonde chi sta cercando di capire cosa gli manca.
  const notaDemo = IS_DEMO
    ? `<p class="challenge-section__nota">${t(BUILD === "beta" ? "betaCareerNote" : "demoCareerNote")}</p>`
    : "";
