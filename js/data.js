 ...
 */
/**
 * Dove mandare chi vuole seguire il gioco.
 *
 * `null` significa **nessun richiamo mostrato**, e questa e' la scelta: un pulsante
 * che porta alla homepage di Steam invece che a una pagina del gioco e' peggio di
 * nessun pulsante — sembra rotto, e chi lo preme non torna.
 *
 * Finche' la pagina Steam non esiste si puo' puntare altrove (la pagina itch, una
 * lista d'attesa): il richiamo si adatta, la destinazione la decidi qui.
 */
export const STORE = {
  // Profilo itch: chi segue riceve la notifica quando esce il gioco completo.
  // Quando la pagina Steam esistera', qui va il suo URL e `kind` diventa "steam" —
  // l'etichetta del pulsante si adatta da se'.
  url: "https://jockeys97.itch.io",
  /**
   * Che tipo di destinazione e', perche' l'etichetta del pulsante cambia: "aggiungi
   * alla wishlist" ha senso solo verso Steam. Puntando altrove con quel testo si
   * prometterebbe una cosa e se ne darebbe un'altra.
   *
   * "steam" → wishlist · "follow" → seguimi per l'uscita
   */
  kind: "follow",
};
...
export const ARENAS = [
  {
    ...
    // La card usa una vista dall'alto coerente con il campo standard giocabile:
    // officina ai bordi, geometria del padel libera e subito leggibile.
    image: "assets/arenas/officina-vapore-standard.webp",
    wallBounce: 0.89,
    floorGrip: 1.0,
 ...
 */
/** Codice di sblocco per demo e collaudo: si inserisce dalla selezione atleti, o
 *  battendo tre volte l'ultima parola del titolo della schermata squadra. */
export const UNLOCK_CODE = "LUCALE";
