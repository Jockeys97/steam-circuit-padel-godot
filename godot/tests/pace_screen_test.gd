...
## The responsive setup grid centres inside the screen with 24 px side padding and a
## 1240 px desktop cap. Its minimum must fit the actual content width available at
## each tested frame, including the demo state where disabled labels are wider.
...
func _strings(audit: AuditBase) -> void:
	...
	for lang_in in Locale.locales():
		...
		audit.check_eq(title_text, Pace.text(ModesScreenClass.PACE_TITLE_KEY, lang).to_upper(),
...
func _layout(audit: AuditBase) -> void:
	for size_in in [FRAME_BIG, FRAME_SMALL]:
		...
		var content_width := minf(SETUP_MAX_WIDTH, frame_size.x) - 2.0 * SETUP_SIDE_PADDING
		audit.check_true(demo_min <= content_width + CONTAINMENT_TOLERANCE,
			"pace/the_rows_minimum_fits_the_responsive_content_box_in_the_demo_view_at_%s" % tag)
