...
func run() -> void:
	...
	check(not Prefs.separate_contexts(store) and Prefs.playback_scope(store, "menu") == "all", "unified library is the default")
	Prefs.Save.save_pref(store, "musicSeparateContexts", true)
	...
	check(music._track == "ost_roster", "legacy split also carries the current song across contexts")
	music.player.stop(0.0)
	music.refresh()
	check(music._track == "ost_training", "natural completion applies the destination legacy playlist")
	...
	juke.free()
	music.free()
	var previous_menu := Prefs.favorites(store, "menu")
	var previous_match := Prefs.favorites(store, "match")
	Prefs.Save.save_pref(store, "musicSeparateContexts", false)
	check(Prefs.favorites(store, "all").has("ost_roster") and Prefs.favorites(store, "all").has("classic_match"), "unified favorites recover both legacy lists")
	Prefs.toggle(store, "all", "classic_match")
	check(not Prefs.favorites(store, "all").has("classic_match"), "unified favorite removals persist")
	check(Prefs.favorites(store, "menu") == previous_menu and Prefs.favorites(store, "match") == previous_match, "unified edits preserve optional legacy lists")
