...
func _timing_presentation() -> void:
	...
	# The two nodes the relation is measured from, reached the way this file reaches
	# every other mark: through the shell's own seam, never a private variable of the
	# shell — the timing presentation lives in `godot/game/court_timing_marks.gd` now
	# and `mount()` hangs its marks on the shell.
	# The module owns the marks' state; the nodes themselves hang on the shell —
	# `mount()` is handed the shell — which is where this file reads them.
	var advice: Label3D = node.get_node_or_null("TimingAdvice") as Label3D
	var ring: MeshInstance3D = node.get_node_or_null("TimingRing") as MeshInstance3D
	check("the marks the proportion is measured on are mounted",
		(advice as Node) != null and (ring as Node) != null, "")
	...
	var word_m: float = float(advice.font_size) * advice.pixel_size if advice != null else 0.0
