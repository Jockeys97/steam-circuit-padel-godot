...
func _initialize() -> void:
	var c := Court.court()
	check(is_equal_approx(Court.court_len(), EXPECTED_WIDTH_M), "width is the declared candidate")
	check(is_equal_approx(Court.court_depth(), 20.0), "length 20 m")
	check(is_equal_approx(Court.world_pos(c.left, c.netY, 0).x, -EXPECTED_WIDTH_M * 0.5), "left glass")
	check(is_equal_approx(Court.world_pos(c.right, c.netY, 0).x, EXPECTED_WIDTH_M * 0.5), "right glass")
