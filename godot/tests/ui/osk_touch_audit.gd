...
##   3. the panel renders that model — label, value preview, six rows, 51 keys, and
...
func _panel_view(audit: AuditBase) -> void:
	...
	_model.open_for("pause/fb-message", "ab", 12, UiStrings.t("volume"))
	_panel.refresh()
	# The grid lays out on the frame after the panel first becomes visible (the
	# visibility flip re-queues the containers' sort); reading the rects in the same
	# frame returns the pre-sort zeros. Probe evidence (`_probe_osk2.gd`): key rects
	# P(0,0) before a frame, P(278,384)/P(871,634) after two. Wait, then read.
	for _i in 2:
		await process_frame
	...
	# 47 characters (`OSK_ROWS`, `js/main.js:430-437`) + the 4 action keys — the count
	# is the reference grid's own, not a literal: the earlier hard-coded 52 was wrong.
	audit.check_eq(targets.size(), "".join(GRID_ROWS).length() + ACTION_IDS.size(), "osk/every_key_of_the_grid_is_a_target")
...
func _touch_events(audit: AuditBase) -> void:
	...
	# Headless: the window extent reverts to the dummy display's own size (64x64) once
	# frames run, and a screen touch outside the window rect never reaches the GUI.
	# Re-assert the frame immediately before injecting — probe evidence
	# (`_probe_touch.gd`): the same event at the same centre records `button_down`
	# with the re-assert and nothing without it.
	root.size = Vector2i(int(FRAME.x), int(FRAME.y))
...
func _platform(audit: AuditBase) -> void:
	...
	audit.note("the shared theme (godot/src/ui/theme/padel_theme.tres) parses as of this wave and carries the palette keys this ticket names; the earlier all-ink fallback note belonged to the pre-fix theme")
