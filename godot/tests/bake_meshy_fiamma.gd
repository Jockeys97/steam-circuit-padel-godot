...
func run():
	...
	var lengths := {"drive":0.62,"smash":0.62,"bandeja":0.54,"backhand":0.62,"slice":0.54,"lunge_forehand":0.80,"wall_exit_forehand":0.72,"forehand_volley":0.50,"backhand_volley":0.50,"forehand_lob":0.60,"backhand_lob":0.60,"ready_stance":3.0}
	var contacts := {"drive":0.34,"smash":0.46,"bandeja":0.38,"backhand":0.34,"slice":0.32,"lunge_forehand":0.30,"wall_exit_forehand":0.34,"forehand_volley":0.42,"backhand_volley":0.42,"forehand_lob":0.36,"backhand_lob":0.36,"ready_stance":0.0}
	...
	var source_contact: float = float(args[1]) if args.size() > 1 and args[1] != "auto" else {"drive":1.0,"smash":1.0,"bandeja":1.15,"backhand":1.25,"slice":1.1,"lunge_forehand":1.1,"wall_exit_forehand":2.05,"forehand_volley":1.0,"backhand_volley":1.1,"forehand_lob":1.0,"backhand_lob":1.12}.get(stroke,1.5)
	...
	# Lobs (2026-09-26) come from the same two sources, the slice being the wrong shape
	# (measured: high backswing, finish LOW). The drive and backhand rise after contact;
	# each is cut while the racket is high and still in front (the drive wraps behind the
	# left shoulder after 1.25 s), over a slower 0.6 s clip so the high finish reads.
	var source_start: float = {"forehand_volley":0.7,"backhand_volley":0.8,"forehand_lob":0.6,"backhand_lob":0.7}.get(stroke, 0.0)
	var source_end: float = {"forehand_volley":1.3,"backhand_volley":1.35,"forehand_lob":1.25,"backhand_lob":1.4}.get(stroke, -1.0)
	...
	var amplitudes: Array = {"forehand_volley":[0.55, 0.2, 0.45],"backhand_volley":[0.6, 0.35, 0.45],"forehand_lob":[0.8, 0.2, 0.6],"backhand_lob":[0.8, 0.3, 0.6]}.get(stroke, [])
	...
	if stroke.begins_with("lunge") or stroke.ends_with("_volley") or stroke.ends_with("_lob"):
		for i in dst.get_bone_count():
	...
	# Amplitude is reduced in SOURCE space, towards the Meshy character's own standing
	# pose at t=0 (arms down), before any retarget: the same reference for every rig.
	# Reducing after the retarget, towards each rig's idle, broke the rigs whose idle
	# frames differ (Pantera/Steamer tipped over; the 24-joint rest spread the arms).
	var calm_global := {}
	var keep_by_bone := {}
	if not amplitudes.is_empty():
		...
		src.force_update_all_bone_transforms()
		for si in src.get_bone_count():
			calm_global[si] = src.get_bone_global_pose(si).basis.get_rotation_quaternion()
			var bn := src.get_bone_name(si)
			var legs := "Leg" in bn or "Foot" in bn or "Toe" in bn
			var free_arm := bn.contains("Left") and ("Arm" in bn or "Hand" in bn or "Shoulder" in bn)
			keep_by_bone[si] = amplitudes[2] if legs else (amplitudes[1] if free_arm else amplitudes[0])
	for frame in range(int(round(clip.length*100))+1):
		...
		for i in dst.get_bone_count():
			...
			if mapping.has(i):
				...
				var src_q := src.get_bone_global_pose(si).basis.get_rotation_quaternion()
				if calm_global.has(si):
					src_q = (calm_global[si] as Quaternion).slerp(src_q, float(keep_by_bone[si])).normalized()
				var delta := src_q * src.get_bone_global_rest(si).basis.get_rotation_quaternion().inverse()
				...
				if dst.get_bone_count() == 24:
					target = src_q * rest_fix.get(si, Quaternion.IDENTITY)
