...
func _build_feedback() -> void:
	...
	var panel := _panel(Vector2(0.5, 1.0), Vector2(-134.0, -190.0), Vector2(268.0, 0.0), "FeedbackPanel")
...
func _build_log() -> void:
	var panel := _panel(Vector2(0.0, 1.0), Vector2(12.0, -182.0), Vector2(420.0, 170.0), "LogPanel")
	var col := VBoxContainer.new()
	...
	col.add_child(title)
	for i in 5:
		var l := _label("", 17, Color(0.86, 0.90, 0.95))
		l.custom_minimum_size = Vector2(384.0, 0.0)
		col.add_child(l)
		_log_lines.append(l)
...
func set_paused(paused: bool) -> void:
	_hint.text = "%s — ESC" % Locale.t("pause") if paused else HINT_PLAY
