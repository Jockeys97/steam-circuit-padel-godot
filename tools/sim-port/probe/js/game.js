  ...
  const grade = timing >= 0.9
    ...
    : passedDistance > lateGrace
      ...
      : timing >= 0.7
        ? "good"
        : "early";
  if (globalThis.__SCP_DBG && String(variant).startsWith("smash")) {
    console.error(`DBGEVAL t=${globalThis.__SCP_TICK} variant=${variant} charge=${charge} aim=${aim} timingAge=${timingAge} aiTiming=${aiTiming} timing=${timing} position=${position} balance=${balance} height=${height} energy=${energy} control=${control} split=${splitStepBonus} quality=${quality} risk=${risk} aggression=${aggression} bx=${ball.x} by=${ball.y} bz=${ball.z} px=${paddle.x} py=${paddle.y} passed=${passedDistance} lateral=${lateralDistance} longitudinal=${longitudinalDistance} reach=${paddle.reach} isPlayer=${paddle.isPlayer} moveRatio=${paddle.moveRatio} motion=${paddle.motion} sprinting=${paddle.sprinting} key=${paddle.key}`);
  }
...
function chooseComputerShot(state, paddle, profile, contactHeight = 0, aiTiming = 1) {
  ...
  if (globalThis.__SCP_DBG) {
    console.error(`DBGCHOOSE t=${globalThis.__SCP_TICK} kind=${kind} baseX=${x} sideBias=${sideBias} ampiezza=${ampiezza} accuracyError=${accuracyError} skill=${profile.skill} lateral=${setup.lateral} margin=${setup.margin} centerX=${centerX} coverageX=${coverageX} bx=${state.ball.x} by=${state.ball.y} bz=${state.ball.z}`);
  }
  return {
    kind,
    x,
...
function applyComputerShot(state, paddle, contactHeight = 0) {
  ...
  if (globalThis.__SCP_DBG) {
    console.error(`DBGAPPLY t=${globalThis.__SCP_TICK} kind=${target.kind} tx=${target.x} ty=${target.y} spread=${executionSpread} risk=${assessment.risk} quality=${assessment.quality} bx=${ball.x} by=${ball.y} bz=${ball.z} ft=${target.flightTime} ps=${powerScale} ap=${atletaPower}`);
  }
