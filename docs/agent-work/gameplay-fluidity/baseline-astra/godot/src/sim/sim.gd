...
static func responder_forecast(paddle: Ent.SimPaddle, ball: Ent.SimBall) -> Dictionary:
	...
	if not ball_playable_direction("player", ball):
		return {"score": INF, "reachable": false, "contactX": paddle.x, "contactZ": 0.0, "lateBy": 0.0, "time": 0.0}
...
## ---------------------------------------------------------------------------
## Tactical contact choice (`js/game.js:552-700`)
## ---------------------------------------------------------------------------
##
## The opponent AI used to strike every ball it could reach (`can_hit`) at any
## height the step allows. That made a defender volley balls a defender has no
## business volleying — stretched wide, late, or above the height `hit_ball`
## clamps a contact to — and it made the bounce and the back glass unreachable
## states for the AI even when they were the better play. This table is the
## deterministic replacement: same inputs on both engines, no RNG, no state of its
## own beyond the one commitment timer the step owns (`State.aiDeferTimer`).
##
##   volley     the ball is inside the paddle's set-up band at a strikeable
##              height and the read delay has elapsed — strike it on the fly;
##   defer      it is not a set-up volley, but it can bounce (or reach the back
##              glass) and come back inside the defender's own band, before a
##              second bounce and inside `AI_DEFER_MAX_TIME` — let it bounce;
##   emergency  nothing better is available and the ball is contactable now;
##   none       the ball is not playable for this side, or a serve is in flight
##              (a receiver may not volley a serve: `can_hit` already refuses it,
##              and the serve is a defer here).
##
## `ai_deferred_contact` is a closed form of the two events that matter — the
## ground bounce (`handle_ground_bounce`) and the back glass (`handle_walls`) —
## with air drag inside a segment ignored exactly like the existing forecasts
## (`ai_responder_forecast`). It changes no state and draws no RNG: it is a
## prediction, the step still decides the actual contact.

## `hit_ball` clamps every contact into `[22, AI_COMFORT_HEIGHT]` (`sim.gd:1546`),
## so above the ceiling a ball is a reach rather than a strike the athlete is set
## up for; below the floor it is at the shoe tops.
const AI_COMFORT_HEIGHT := 74.0
const AI_COMFORT_FLOOR := 34.0
## The fraction of the paddle's own contact band that still reads as a set-up
## shot. Past it the ball is at the fingertips.
const AI_COMFORT_REACH := 0.62
## What counts as "comfortable enough to volley", and how much a stronger athlete
## lowers the bar. Both scale off `paddle.skill`, which the difficulty ladder
## already sets; the reaction delay, the receiver lock and the seeded wrong-foot
## draws are untouched by this choice.
const AI_COMFORT_THRESHOLD := 0.7
const AI_COMFORT_SKILL_GAIN := 0.16
## The top of the reach band the forecasts already use, and the tolerance they
## accept for a late arrival (`ai_responder_forecast`).
const AI_HIT_HEIGHT_CEILING := 86.0
const AI_DEFER_MARGIN := 0.12
## A deferred contact has to arrive inside this horizon, the same one the AI
## forecast uses. A commitment is held until the ball has bounced, and this is how
## long after the predicted contact the hold is allowed to survive a prediction
## that drifts.
const AI_DEFER_MAX_TIME := 1.8
const AI_DEFER_RELEASE := 0.25


## How far `z` is from a height an athlete strikes from: `hit_ball` clamps every
## contact into `[22, AI_COMFORT_HEIGHT]`, so above the ceiling a ball is a reach
## rather than a strike, and below the floor it is at the shoe tops.
static func ai_height_penalty(z: float) -> float:
	if z > AI_COMFORT_HEIGHT:
		return clampf(
			(z - AI_COMFORT_HEIGHT) / maxf(1.0, AI_HIT_HEIGHT_CEILING - AI_COMFORT_HEIGHT), 0.0, 1.0)
	if z < AI_COMFORT_FLOOR:
		return clampf((AI_COMFORT_FLOOR - z) / AI_COMFORT_FLOOR, 0.0, 1.0)
	return 0.0


## The first contact the AI would actually make if it took this ball on the fly:
## the moment the ball is inside its racket band, down to a strikeable height, and
## still before it bounces. This is the contact the volley/bounce choice is about.
## `ai_responder_forecast` answers a different question — where the ball meets the
## paddle's own y line — and a defender with a depth reach meets the ball earlier
## and higher than that, which is exactly the reach this decision has to see.
static func ai_volley_contact(state: State, paddle: Ent.SimPaddle, ball: Ent.SimBall) -> Dictionary:
	var balance: Dictionary = Frozen.balance()
	var court: Dictionary = Frozen.court()
	var g: float = float(balance["ballGravity"])
	var miss := {
		"valid": false, "contactX": paddle.x, "contactY": paddle.y, "contactZ": 0.0,
		"time": INF, "lateBy": 0.0,
	}
	if not ball_playable_direction("ai", ball) or ball.serveInFlight or ball.netFaultOwner != null:
		return miss
	# The arc ends at the ground: after the bounce this is no longer a volley.
	var t_ground: float = INF
	if ball.z > 0.0 or ball.vz > 0.0:
		t_ground = (ball.vz + sqrt(maxf(0.0, ball.vz * ball.vz + 2.0 * g * maxf(0.0, ball.z)))) / g
	var horizon: float = minf(t_ground, AI_DEFER_MAX_TIME)
	if horizon <= 0.0:
		return miss
	# Down to a height a contact is allowed at: the descending crossing of the
	# playable ceiling, or now when the ball is already under it.
	var t_playable: float = 0.0
	var playable_height: float = float(balance["playableHitHeight"])
	var hi_disc: float = ball.vz * ball.vz + 2.0 * g * (ball.z - playable_height)
	if ball.z > playable_height and hi_disc >= 0.0:
		t_playable = (ball.vz + sqrt(hi_disc)) / g
	# Inside the racket's depth band, which the ball enters and leaves once.
	var depth_band: float = maxf(1.0, paddle.reach * float(balance["aiDepthReach"]))
	var t_band: float = 0.0
	if absf(ball.vy) > 0.001:
		var entry_a: float = (paddle.y + depth_band - ball.y) / ball.vy
		var entry_b: float = (paddle.y - depth_band - ball.y) / ball.vy
		if maxf(entry_a, entry_b) < 0.0:
			return miss
		t_band = maxf(0.0, minf(entry_a, entry_b))
	elif absf(ball.y - paddle.y) > depth_band:
		return miss
	var t_contact: float = maxf(t_playable, t_band)
	if t_contact > horizon:
		return miss
	var contact_y: float = ball.y + ball.vy * t_contact
	if contact_y < float(court["top"]) + ball.r or contact_y > float(court["netY"]):
		return miss
	var contact_x: float = reflected_court_x(ball.x + ball.vx * t_contact)
	var travel: float = maxf(0.0, absf(contact_x - paddle.x) - contact_width(paddle, ball))
	var late_by: float = maxf(0.0, travel / maxf(1.0, paddle.speed) - t_contact)
	return {
		"valid": true,
		"contactX": contact_x,
		"contactY": contact_y,
		"contactZ": maxf(0.0, ball.z + ball.vz * t_contact - 0.5 * g * t_contact * t_contact),
		"time": t_contact,
		"lateBy": late_by,
	}


## How set up the AI is for that volley contact: `1` is a strike at a set-up
## height with time to spare, `0` is the top of the reach or a ball it can only
## just arrive at. Height and available time are the two things a defender reads
## before choosing between the volley and the bounce.
static func ai_volley_comfort(volley: Dictionary) -> float:
	if not bool(volley["valid"]):
		return 0.0
	var late_penalty: float = clampf(float(volley["lateBy"]) / AI_DEFER_MARGIN, 0.0, 1.0)
	return clampf(1.0 - maxf(ai_height_penalty(float(volley["contactZ"])), late_penalty), 0.0, 1.0)


## How set up the AI is for the contact it could make RIGHT NOW: the ball is
## inside the racket band at a height it can strike. Used for the ball that has
## already arrived, where the trajectory forecast has nothing left to say.
##
## Only the LATERAL spread counts here. Depth is where the ball chose to come to
## the defender, not a fault in the defender's position — the arrival it can and
## cannot get to is `ai_contact_comfort`'s job, through the forecast's travel
## budget. Reading depth as discomfort here made every ball met in front of the
## AI look like a stretch, which is the bug this whole decision exists to fix.
static func ai_live_comfort(paddle: Ent.SimPaddle, ball: Ent.SimBall) -> float:
	var width: float = maxf(1.0, contact_width(paddle, ball))
	var spread: float = clampf(absf(ball.x - paddle.x) / width, 0.0, 1.0)
	var spread_penalty: float = clampf((spread - AI_COMFORT_REACH) / (1.0 - AI_COMFORT_REACH), 0.0, 1.0)
	return clampf(1.0 - maxf(spread_penalty, ai_height_penalty(ball.z)), 0.0, 1.0)


## The ground bounce, applied exactly as `handle_ground_bounce` applies it, to a
## predicted impact. Returns the post-bounce position and velocity.
static func ai_predicted_bounce(ball: Ent.SimBall, px: float, py: float, vx: float, vy: float,
		impact_vz: float) -> Dictionary:
	var balance: Dictionary = Frozen.balance()
	...
	return {
		"x": px,
		"y": py,
		"vx": (vx + ball.spin * float(balance["groundSpinTransfer"])) * float(balance["groundTangentialDamping"]),
		"vy": vy * float(balance["groundTangentialDamping"]) * (1.0 - slice_amount * 0.14) * (1.0 + topspin_amount * 0.08),
		"vz": maxf(minimum_bounce, impact_speed * restitution),
	}


## The first contact this paddle can still make inside ONE arc of the ball's
## flight: the ball coming DOWN into `[AI_COMFORT_FLOOR, AI_COMFORT_HEIGHT]` while
## it is still on the AI's side of the net and inside the walls, with the travel
## time to be there. A ball above the ceiling is deliberately not a candidate —
## that is the reach the AI is supposed to decline — so the earliest candidate is
## the descending crossing. `{"valid": false}` when the arc offers no such moment.
static func ai_deferred_segment(state: State, paddle: Ent.SimPaddle, ball: Ent.SimBall,
		px: float, py: float, pz: float, vx: float, vy: float, vz: float,
		t0: float, bounces: int, glass: bool) -> Dictionary:
	var balance: Dictionary = Frozen.balance()
	var court: Dictionary = Frozen.court()
	var g: float = float(balance["ballGravity"])
	var miss := {
		"valid": false, "contactX": paddle.x, "contactY": paddle.y, "contactZ": 0.0,
		"time": INF, "afterBounce": bounces > 0, "glass": glass,
	}
	# The arc ends at the ground — a second bounce is the end of the point — or at
	# the back glass, which the caller reflects and walks again.
	var t_end: float = INF
	if pz > 0.0 or vz > 0.0:
		t_end = (vz + sqrt(maxf(0.0, vz * vz + 2.0 * g * maxf(0.0, pz)))) / g
	if vy < 0.0:
		t_end = minf(t_end, maxf(0.0, (float(court["top"]) + ball.r - py) / vy))
	var horizon: float = minf(t_end, AI_DEFER_MAX_TIME - t0)
	if not is_finite(t_end) or horizon <= 0.0:
		return miss
	# When the ball is at or below the ceiling: a ball already under it qualifies
	# now, one that goes over it qualifies again on the way down.
	var t_open: float = 0.0
	var hi_disc: float = vz * vz + 2.0 * g * (pz - AI_COMFORT_HEIGHT)
	if pz > AI_COMFORT_HEIGHT and hi_disc >= 0.0:
		t_open = (vz + sqrt(hi_disc)) / g
	# ... and above the floor: the last moment it is still a strike rather than a
	# scoop, plus the first moment it is one when the ball starts below it.
	var t_close: float = horizon
	var lo_disc: float = vz * vz + 2.0 * g * (pz - AI_COMFORT_FLOOR)
	if lo_disc >= 0.0:
		if pz >= AI_COMFORT_FLOOR:
			t_close = minf(horizon, (vz + sqrt(lo_disc)) / g)
		else:
			t_open = maxf(t_open, (vz - sqrt(lo_disc)) / g)
	if t_open >= t_close:
		return miss
	# The ball has to still be on the AI's side of the net and inside the walls
	# when it is struck: a contact past the net line is the opponent's ball, and a
	# contact past the back wall is one the glass has already taken.
	var y_near: float = float(court["netY"]) - 4.0
	var y_far: float = float(court["top"]) + ball.r
	var side_a: float = 0.0
	var side_b: float = INF
	if absf(vy) > 0.001:
		if vy < 0.0:
			side_a = (y_near - py) / vy
			side_b = (y_far - py) / vy
			...
			side_a = (y_far - py) / vy
			side_b = (y_near - py) / vy
	elif py < y_far or py > y_near:
		return miss
	var t_contact: float = maxf(t_open, maxf(0.0, side_a))
	if t_contact > minf(t_close, side_b):
		return miss
	var contact_x: float = reflected_court_x(px + vx * t_contact)
	var contact_y: float = py + vy * t_contact
	var contact_z: float = maxf(0.0, pz + vz * t_contact - 0.5 * g * t_contact * t_contact)
	# The window above is built from the two crossings, but a ball that never rises
	# to the floor has no crossing at all — and a contact at the shoe tops is a
	# scramble, not something to wait for. Refuse it here, so the caller strikes the
	# ball instead of holding a deferral it cannot pay.
	if contact_z < AI_COMFORT_FLOOR or contact_z > AI_COMFORT_HEIGHT:
		return miss
	# The paddle crosses and backs up at different rates (`move_paddle_to`), so the
	# travel budget is the larger of the two, not their sum.
	var speed: float = maxf(1.0, paddle.speed)
	var depth_band: float = maxf(1.0, paddle.reach * float(balance["aiDepthReach"]))
	var lateral: float = maxf(0.0, absf(contact_x - paddle.x) - contact_width(paddle, ball))
	var depth: float = maxf(0.0, absf(contact_y - paddle.y) - depth_band)
	var late_by: float = maxf(lateral / speed, depth / (speed * 0.56)) - (t0 + t_contact)
	if late_by > AI_DEFER_MARGIN:
		return miss
	return {
		"valid": true,
		"contactX": contact_x,
		...
		"contactZ": contact_z,
		"time": t0 + t_contact,
		"afterBounce": bounces > 0,
		"glass": glass,
	}


## Where the defender can still meet this ball after it has bounced on its own
## side — or after the back glass. Walks at most one ground bounce and one glass
## rebound; returns `{"valid": false}` when the bounce lands outside the AI court,
## when a second bounce would come first, or when the resulting contact is out of
## reach. This is what lets the AI decline a stretched volley without ever waiting
## past a second bounce.
static func ai_deferred_contact(state: State, paddle: Ent.SimPaddle, ball: Ent.SimBall) -> Dictionary:
	var balance: Dictionary = Frozen.balance()
	var court: Dictionary = Frozen.court()
	var g: float = float(balance["ballGravity"])
	var px: float = ball.x
	...
	var bounces: int = int(ball.bounces["ai"])
	var glass := false
	var miss := {
		"valid": false, "contactX": paddle.x, "contactY": paddle.y, "contactZ": 0.0,
		"time": INF, "afterBounce": bounces > 0, "glass": glass,
	...
	for _event in 3:
		# The current arc counts too: a ball still above the strike window offers a
		# contact on the way down, and waiting for that is the same decision as
		# waiting for a bounce — it is the ball the AI is set up for, one tick later.
		var hit: Dictionary = ai_deferred_segment(
			state, paddle, ball, px, py, pz, vx, vy, vz, elapsed, bounces, glass)
		if bool(hit["valid"]):
			return hit
		# The next event in this arc: the ground, or the back glass, whichever the
		# ball reaches first.
		var t_ground: float = INF
		if pz > 0.0 or vz > 0.0:
			t_ground = (vz + sqrt(maxf(0.0, vz * vz + 2.0 * g * maxf(0.0, pz)))) / g
		var t_wall: float = INF
		if vy < 0.0:
			t_wall = (float(court["top"]) + ball.r - py) / vy
		if t_wall < t_ground:
			# Off the back glass and back into the court, with the same reflection
			# `handle_walls` applies. One glass per prediction is enough: a second
			# would be a different ball.
			if glass or t_wall <= 0.0 or elapsed + t_wall > AI_DEFER_MAX_TIME:
				return miss
			py = float(court["top"]) + ball.r
			px = reflected_court_x(px + vx * t_wall)
			pz = maxf(0.0, pz + vz * t_wall - 0.5 * g * t_wall * t_wall)
			vz = vz - g * t_wall
			vy = -vy * float(state.arena["wallBounce"])
			vx = vx * float(balance["wallTangentialDamping"])
			elapsed += t_wall
			glass = true
			continue
		if not is_finite(t_ground) or t_ground <= 0.0 or elapsed + t_ground > AI_DEFER_MAX_TIME:
			return miss
		var impact_vz: float = vz - g * t_ground
		var bounce: Dictionary = ai_predicted_bounce(ball, px, py, vx, vy, impact_vz)
		px = float(bounce["x"])
		py = float(bounce["y"])
		vx = float(bounce["vx"])
		vy = float(bounce["vy"])
		...
		elapsed += t_ground
		bounces += 1
		# A bounce that lands off the AI's court, or a second bounce at all, ends
		# the point: there is no contact to wait for.
		if bounces > 1 or py < float(court["top"]) or py > float(court["netY"]):
			return miss
	return miss


## The AI's choice for the ball it is defending. `"volley"` and `"emergency"` mean
## "strike it this tick"; `"defer"` means "hold position for `contactX` and let it
## bounce"; `"none"` means there is nothing to do (out of the AI's reach, or a
## serve still in flight). No RNG, no writes: `update_doubles_ai` owns the timer
## and the contact.
static func ai_contact_plan(state: State, paddle: Ent.SimPaddle, ball: Ent.SimBall) -> Dictionary:
	var volley: Dictionary = ai_volley_contact(state, paddle, ball)
	var comfort: float = ai_volley_comfort(volley)
	var plan := {
		"action": "none", "comfort": comfort, "contactX": paddle.x, "contactY": paddle.y,
		"contactZ": ball.z, "time": 0.0, "afterBounce": false, "glass": false,
		"reason": "not-playable",
	}
	if not ball_playable_direction("ai", ball) or ball.serveInFlight or ball.netFaultOwner != null:
		return plan
	var ready: bool = state.aiReactionDelay <= 0.0
	var now_ok: bool = can_hit(paddle, ball)
	var live_comfort: float = ai_live_comfort(paddle, ball) if now_ok else 0.0
	var threshold: float = AI_COMFORT_THRESHOLD - clampf(paddle.skill, 0.0, 1.0) * AI_COMFORT_SKILL_GAIN
	var deferred: Dictionary = ai_deferred_contact(state, paddle, ball)
	var set_up: bool = now_ok and live_comfort >= threshold
	var arrival: bool = comfort >= threshold
	# A commitment holds while the ball has not bounced: once the AI has decided to
	# play this one off the ground it does not change its mind about a ball it is
	# not set up for. It releases the moment the ball bounces, when the prediction
	# stops being valid, or when the ball turns into a set-up volley after all —
	# the commitment is there to stop the AI dithering over a stretch, not to make
	# it refuse an easy ball.
	var committed: bool = state.aiDeferTimer > 0.0 \
		and int(ball.bounces["ai"]) == 0 \
		and bool(deferred["valid"]) \
		and not set_up \
		and not arrival
	if committed:
		plan["action"] = "defer"
		plan["reason"] = "committed"
		plan["contactX"] = float(deferred["contactX"])
		plan["contactY"] = float(deferred["contactY"])
		plan["contactZ"] = float(deferred["contactZ"])
		plan["time"] = float(deferred["time"])
		plan["afterBounce"] = bool(deferred["afterBounce"])
		plan["glass"] = bool(deferred["glass"])
		return plan
	# The ball is here, and the contact is one an athlete is set up for.
	if set_up:
		plan["action"] = "volley"
		plan["reason"] = "set-up"
		plan["contactX"] = ball.x
		plan["contactY"] = ball.y
		plan["contactZ"] = ball.z
		plan["comfort"] = live_comfort
		return plan
	# Still coming, and the arrival the forecast sees is a set-up volley: take it
	# in the air as it arrives rather than letting it bounce.
	if arrival:
		plan["action"] = "volley"
		plan["reason"] = "arriving"
		return plan
	# Not a volley to plan for: let it bounce when the bounce is reachable, and
	# otherwise strike the uncomfortable ball anyway — the last contact is still
	# better than no contact at all.
	if bool(deferred["valid"]):
		plan["action"] = "defer"
		plan["reason"] = "let-it-bounce"
		plan["contactX"] = float(deferred["contactX"])
		plan["contactY"] = float(deferred["contactY"])
		plan["contactZ"] = float(deferred["contactZ"])
		plan["time"] = float(deferred["time"])
		...
		return plan
	if now_ok and ready:
		plan["action"] = "emergency"
		plan["reason"] = "last-contact"
		plan["contactX"] = ball.x
		plan["contactY"] = ball.y
		plan["contactZ"] = ball.z
		plan["comfort"] = live_comfort
		return plan
	plan["reason"] = "out-of-reach"
	return plan
...
static func update_doubles_ai(state: State, dt: float) -> void:
	...
	state.aiDeferTimer = maxf(0.0, state.aiDeferTimer - dt)
	...
	for paddle in defenders:
		if can_hit(paddle, ball):
			responder = paddle
			break
	# The choice between a volley and letting the ball bounce is made for the
	# defender that owns the ball — the locked primary, or whoever can reach it —
	# and it is made even when the ball is not contactable yet: a ball still above
	# the reach band is exactly the one a defender has to decide about early.
	var planner: Ent.SimPaddle = state.paddle(state.aiPrimaryKey)
	if responder != null:
		planner = responder
	if planner != null:
		var plan: Dictionary = ai_contact_plan(state, planner, ball)
		var action := String(plan["action"])
		if action == "defer":
			state.aiDeferTimer = maxf(
				state.aiDeferTimer, minf(AI_DEFER_MAX_TIME, float(plan["time"]) + AI_DEFER_RELEASE))
			# Position for the contact the AI has committed to, not for the ball's
			# current x: that is the whole point of declining the volley.
			state.aiTargetX = clampf(
				float(plan["contactX"]), float(court["left"]) + 72.0, float(court["right"]) - 72.0)
		else:
			state.aiDeferTimer = 0.0
			if (action == "volley" or action == "emergency") and controllable_height and state.aiReactionDelay <= 0.0:
				hit_ball(state, planner, 0.88 + float(ai["skill"]) * 0.12)
